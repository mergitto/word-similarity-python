...
def recommend_rate(reports_values, reports):
    ...
    for report_no in reports_values:
        ...
        if recommend_formula == 1:
            recommend_rate = simSumNorm[report_no] * (typeRate * shokushuRate) + reports[report_no]["is_high_predicted"]
        elif recommend_formula == 2:
            recommend_rate = simSumNorm[report_no] * (typeRate * shokushuRate) + reports[report_no]["feature_importance_rate_norm"]
        else:
